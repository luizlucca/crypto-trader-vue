...
│ Vue workspace ─ MarketChart (canvas) ─ OrderBook (DOM imperativo)     │
└──────────────────────┬─────────────────────────────────────────────────┘
...
└───────────────┬──────────────────────────────────┬─────────────────────┘
 ...
 candles + livro + histórico              exchangeInfo + ticker/24hr
 RxJS + WebSocket                         cache 1h + singleflight
                │                                  │
                ▼                                  ▼
...
outro para catálogo. Cada um possui timeout, falha de pendências, reinício
automático e, no caso realtime, recuperação da assinatura ativa após crash.
O encerramento da aplicação finaliza ambos explicitamente.
...
workspace ou da pesquisa.

Favoritos são persistidos no `localStorage` e separados por provedor, mercado
e símbolo. As janelas sincronizam favoritos por IPC. Ao trocar de par, o
workspace cancela a sessão anterior, substitui o histórico uma vez com
`setData()` e retoma atualizações incrementais com `series.update()`.
