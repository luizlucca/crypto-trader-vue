...
import type { SecurityPreferences, SecuritySnapshot } from '@shared/contracts/security'
  ...
  <section class="settings-content security-preferences-panel">
    <div class="settings-section-heading">
      <div>
        ...
        <p>
          Gráfico, livro e dados públicos continuam ativos quando a conta é bloqueada.
        </p>
      </div>
      <span class="security-panel-status">
        <ShieldCheck aria-hidden="true" />
        {{ snapshot.state === 'unlocked' ? 'Cofre desbloqueado' : 'Cofre bloqueado' }}
      </span>
    </div>

    <form class="security-preferences-form" @submit.prevent="save">
      <label>
        <input v-model="draft.lockOnMinimize" :disabled="pending" type="checkbox">
        <span>
      ...
      <label>
        <input v-model="draft.lockOnSuspend" :disabled="pending" type="checkbox">
        <span>
          <strong>Bloquear ao suspender</strong>
          <small>
            Também se aplica ao bloqueio de sessão quando o sistema oferece o evento.
            <template v-if="isLinux"> Indisponível para lock-screen no Linux.</template>
          </small>
        </span>
      </label>
      ...
      <label class="security-preference-select">
        <span><MonitorDown aria-hidden="true" /> Ao fechar a janela principal</span>
      ...
      <p v-if="error" class="security-access-error" role="alert">{{ error }}</p>
      <footer>
